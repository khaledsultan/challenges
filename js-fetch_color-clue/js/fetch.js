...
export async function fetchNewColor() {
  ...
  let colorApiUrl = `https://www.thecolorapi.com/id?hex=${hexCode}`;
  /**
   * Hint 1:
  ...
  // --v-- your code here --v--
  try {
    let response = await fetch(colorApiUrl);

    if (response.ok) {
      // Success (Good Response)
      let data = await response.json();
      let newColor = data.hex.value;
      var closestColor = data.name.closest_named_hex;
      var closestColorName = data.name.value;

      // console.log(closestColor);
      // console.log(newColor);
      setColorToGuess(closestColor, closestColorName);
    } else {
